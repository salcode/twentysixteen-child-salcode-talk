 ...
 *
 * @package WordPress
 * @subpackage Twenty_Sixteen_child_salcode_talk
...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<?php
		 ...
		 */
		$post_id = get_the_ID();
		$color   = get_post_meta( $post_id, 'fe_color', true );
		if ( ! $color ) {
	...
	<header class="entry-header" style="color: <?php echo esc_attr( $color ); ?>;">
		...
		$post_id = get_the_ID();
		$subtitle = get_post_meta( get_the_ID(), 'fe_subtitle', true );
		if ( $subtitle ) {
			echo '<h2 class="fe-entry-subtitle">' . esc_html( $subtitle ) . '</h2>';
		}
		?>
	</header><!-- .entry-header -->
	...
	<div class="entry-content">
		<?php
			...
			$post_id = get_the_ID();
			$url = get_post_meta( $post_id, 'fe_learn_more', true );

			if ( $url ) {

				printf( '<a class="fe-btn fe-btn-learn-more" href="%1$s">%2$s</a>',
